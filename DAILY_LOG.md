...
| #5 分段透镜→factor_audit | ❌ **仍欠**（唯一真欠的旧任务）；spec 已写，待拍 🔴 现代段切法 |

### ③ 待办 / 未决（均非缺陷——审查缺陷已清零）
- [ ] **#5 分段透镜推广到 factor_audit**：spec `SPEC_FACTOR_SEGMENT.md` 在；**动手前先定 🔴** 现代段怎么切（A/B/C + 年限）——含判断点，先问用户
- [ ] **主盘 dashboard.html 的 Phase2/3 双语**（新工具页已双语，主盘还没）
- [ ] **v1.5 自生长自动发现闭环**：发现候选→质量门→并入登记簿/坟场→度量（现仅展示层）
