...
def build_features(df):
    ...
    # AI景气（NVDA相对强度）
    if "NVDA" in m.columns:
        feat["nvda_mom3m"]  = m["NVDA"].pct_change(3)
        feat["nvda_mom12m"] = m["NVDA"].pct_change(12)
        if "NASDAQ" in m.columns:
            feat["nvda_vs_nasdaq"] = m["NVDA"].pct_change(3) - m["NASDAQ"].pct_change(3)  # AI超额收益
