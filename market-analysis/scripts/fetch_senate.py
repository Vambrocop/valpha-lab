...
⚠️ 2026-06-30 复核（有意标注·别再当"掉队的破功能"）：**上游源已停更**——该仓库最新交易日 2019-12，
本地 csv 止于 2020-11。故本族是**完结的历史研究(2012–2020)，不是实时信号**：
  · 故意**不进 run_all / workflow**——再抓也是同一份死数据，每日重抓是假逻辑。
  · 结论已定且诚实（跟议员披露后买 ≈ 打平，见 senate_signal.json）；改方法论才手动重跑这两个脚本。
  · 要做**实时**政治钱信号需换活源（Quiver/Unusual Whales 等，多为付费；免费活源稀缺）——属新立项。
"""
import json
import csv
import re
