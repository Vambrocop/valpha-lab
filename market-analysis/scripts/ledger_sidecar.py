...
会合法去重排序重封，前缀不变式不适用）、overreaction_signal_log（在 processed/ merge=union
区，git 并集合并可合法重排行）不在此列——见各自注释。
"""
import csv
import datetime
import sys
...
def seal_all(data_dir=DATA, manifest=MANIFEST, specs=SPECS, *,
    ...
    for fname, core_spec in specs:
        ...
        status, rec, msg = seal_one(p, core_spec, latest.get(fname), rebless_note=rn)
        if status == "refuse":
